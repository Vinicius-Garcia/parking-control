...
from classes.registerclass import UserRegistrationApp
...
class App:
    ...
    def show_login_screen(self):
        ...
        self.rt.after(0, lambda: self.rt.state('zoomed'))
        ...
        def login():
            ...
            if user:
                self.show_menu(user_type)
            else:
                label_status.config(text="Login failed. Please try again.")

        def switch_to_register_screen():
            UserRegistrationApp(self.rt)


        fr = customtkinter.CTkFrame(master=self.rt)
    ...
    def show_register_screen(self):
        def add_to_database():
            ...
            # Continue with adding data to the database
            cursor.execute('''
            ...
            # Optionally, switch back to the login screen
    ...
    def entry_menu(self):
        ...
        def open_entry_details(event):
            ...
            details_window = tk.Toplevel(self.rt)
        ...
        entry1 = customtkinter.CTkEntry(search, width=240, height=32, placeholder_text="PLACA", validate="key",
                                        validatecommand=(self.rt.register(validate_length), '%P'))
    ...
    def exit_menu(self):
        ...
        def open_entry_details(selected_item):
            ...
            details_label = customtkinter.CTkLabel(
                details_frame, width=120, height=1,
                text=f"Tempo desde a entrada: {hours:02d}:{minutes:02d}:{seconds:02d}",
                font=("Roboto", 16), anchor='w')
            details_label.pack(pady=6, padx=10, anchor="w")
            ...
            button = customtkinter.CTkButton(details_frame, width=240, height=32, text="DAR SAIDA",
                                             command=lambda: move_to_history(selected_entry, formatted_time,
                                                                             formatted_saida, time_difference,
                                                                             pagamento))
            button.pack(pady=12, padx=10)
        ...
        def open_entry_details_button(selected_item):
            print("Received selected item:", selected_item)
            print(tree.selection())
            details_window = tk.Toplevel(rt)
            details_window.title("Entry Details")
        ...
        def dar_saida():
            plate = entry1.get()  # Get the entered license plate
            print("Plate:", plate)
            cursor.execute("SELECT placa, data FROM entry WHERE placa=?", (plate,))
            entry = cursor.fetchone()
            print(entry)

            if entry:
                open_entry_details_button(entry)
            else:
        ...
        button = customtkinter.CTkButton(search, width=240, height=32, text="DAR SAIDA", command=dar_saida)
        button.pack(pady=12, padx=10, side="left")

        entry1.bind("<Return>", enter_pressed)
        self.rt.after(100, set_focus)
    ...
    def config(self):
        ...
        def price():
            ...
            price_window = tk.Toplevel(self.rt)
        ...
        def texts():
            def add_user():
                adddetails_window = tk.Toplevel(self.rt)
                adddetails_window.title("Adicionar Usuário")
                adddetails_window.geometry("400x450")
                adddetails_window.configure(bg="#212121")
            ...
            user_window = tk.Toplevel(self.rt)
    ...
    def report(self):
        ...
        def update_entry_list(start_date, end_date):
            try:
                ...
                for entry in entries:
                    ...
                    tree.insert('', tk.END,
                                values=(placa, data_entrada, data_saida, tempo_estadia, valor_total, pagamento))
        ...
        def export_pdf():
            try:
                ...
                header = (
                "Placa", "Data de Entrada", "Data de Saída", "Tempo de Permanência", "Valor Pago", "Pagamento")
                data.append(header)
        ...
        start_date_entry = DateEntry(datesFrame, width=16, background="magenta3", foreground="white", bd=2,
                                     locale="pt_br")
        ...
        end_date_entry = DateEntry(datesFrame, width=16, background="magenta3", foreground="white", bd=2,
                                   locale="pt_br")
        end_date_entry.pack(padx=(0, 40), pady=5, anchor="w", side="left")

        generate_button = customtkinter.CTkButton(datesFrame, width=140, height=40, text="GERAR",
