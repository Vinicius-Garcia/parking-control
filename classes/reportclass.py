...
class Report(customtkinter.CTk):
    def __init__(self):
        ...
        self.setup_ui()
        self.total_frame = customtkinter.CTkFrame(master=self.total)
    ...
    def setup_ui(self):
        ...
        self.total = customtkinter.CTkFrame(master=fr)
        self.total.pack(pady=10, padx=10, anchor="e")

        self.generate = customtkinter.CTkFrame(master=fr)
        self.generate.pack(pady=10, padx=10, anchor="e")

        self.button = customtkinter.CTkButton(self.generate, width=120, height=24, text="EXPORTAR PDF",
                                              command=self.export_pdf)
        self.button.pack(padx=10, pady=10, side="left")

        self.button1 = customtkinter.CTkButton(self.generate, width=120, height=24, text="EXPORTAR EXCEL",
    ...
    def generate_report(self):
        ...
        # Calculate total values for different payment methods
        total_pix = 0
        total_cash = 0
        total_card = 0

        for entry in self.tree.get_children():
            print(self.tree.item(entry)['values'])
            values = self.tree.item(entry)['values']
            valor_total = float(values[4])  # Assuming value is stored at index 4
            print(valor_total)
            pagamento = values[5]  # Assuming payment type is stored at index 5
            print(pagamento)
            if pagamento == "PIX":
        ...
        self.total_card_label.configure(text=f"Cartão: R${total_card:.2f}")
